  ...
  <v-app>
    <v-app-bar app dark flat>
      ...
      <v-toolbar-items>
        ...
        <v-btn text dark v-if="!user && login_enabled" :to="{name: 'login'}" color="success">
    ...
    <v-main>
      ...
      <div class="rent-buttonarea">
        ...
        <v-btn rounded x-large color="success" class="login-button" v-if="!user && login_enabled" :to="{name: 'login'}">
          <v-icon>{{ mdi.login }}</v-icon>&nbsp;<span>{{ $t('message.app.login') }}</span>
        </v-btn>
        <RentButton v-if="user && login_enabled" class="rent-button" />
      </div>
      <AppError />
    </v-main>
  </v-app>
